/**
 * Copyright 2016 fedapay.com, Inc. or its affiliates. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License").
 * You may not use this file except in compliance with the License.
 * A copy of the License is located at
 *
 *  http://aws.fedapay.com/apache2.0
 *
...
define(
    ...
    function ($, Class, $t) {
        ...
        return Class.extend({
                ...
                defaults: {
                    $fedapayFields: null,
                    $fedapayCredentialJson: null,
                    $fedapaySpBack: null,
                    $fedapayMerchantId: null,
                    selector: 'fedapay_payment',
                    $container: null,
                    pollInterval: 1500,
                    $form: null,
                    apSimplePathSelector: '#fedapay_simplepath',
                    apSimplePathBackSelector: '#fedapay_simplepath_back',
                },
                ...
                initObservable: function () {
                    ...
                    self.$fedapaySimplepath = $(self.apSimplePathSelector);
                    self.$fedapayFields = $('#payment_' + self.getCountry() + '_' + self.selector + ' .form-list');
                    self.$fedapayCredentialsHeader = $('#payment_' + self.getCountry() + '_' + self.selector
                        + '_credentials-head');
                    self.$fedapayCredentialJson = $('#payment_' + self.getCountry() + '_' + self.selector
                        + '_credentials_credentials_json');
                    self.$fedapayMerchantId = $('#payment_' + self.getCountry() + '_' + self.selector
                        + '_credentials_merchant_id').val();
                    self.$fedapaySpBack = $(self.apSimplePathBackSelector);
                    self.$container = $(self.apSimplePathSelector);

                    if (this.isMultiCurrencyRegion) {
                        $('#row_payment_' + self.getCountry() + '_fedapay_payment_advanced_sales_options_multicurrency').show();
                        $('#row_payment_other_fedapay_payment_advanced_sales_options_multicurrency').show();
                    }
                    else {
                        $('#row_payment_' + self.getCountry() + '_fedapay_payment_advanced_sales_options_multicurrency').hide();
                        $('#row_payment_other_fedapay_payment_advanced_sales_options_multicurrency').hide();
                    }
                    
                    if (self.$fedapayMerchantId) {
                        self.hidefedapayConfig();
                    }
                    else {
                        self.showfedapayConfig();
                    }
                ...
                initEventHandlers: function () {
                    ...
                    self.$fedapaySpBack.click(function () {
                        self.showfedapayConfig();
                        return false;
                    });
                    ...
                    self.$fedapayCredentialJson.on('input', function () {
                ...
                updateCredentials: function (self) {
                    var elJson = self.$fedapayCredentialJson.val(), obj = null, success = true, item = null;
                    ...
                    try {
                        ...
                        obj = null;
                        self.$fedapayCredentialJson.val('').attr(
                            'placeholder',
                    ...
                    if (obj && typeof obj === 'object') {
                        ...
                        for (var prop in obj) {
                            if (obj.hasOwnProperty(prop)) {
                                item = $('#payment_' + self.getCountry() + '_fedapay_payment_credentials_'
                                    + $.trim(prop));

                                if (item && item.length) {
                                    $('#payment_' + self.getCountry() + '_fedapay_payment_credentials_'
                ...
                /**
                 * Sets up fedapay merchant key popup and polls for data update upon user completion.
                 */
                setupWindowLaunch: function () {
                    var self = this,
                        ...
                        elCheckDefault = $('#payment_' + self.getCountry()
                            + '_fedapay_payment_credentials_payment_region_inherit:checked'),
                        elRegion = $('payment_' + self.getCountry() + '_fedapay_payment_credentials_payment_region'),
                        elJson = self.$fedapayCredentialJson.val();
                    ...
                    self.launchPopup(self.fedapayUrl, popupWidth, popupHeight);
                    ...
                    if (windowOpen) {
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                if (response) {
                                    $('#fedapay_reload').show();
                                    document.location.replace(document.location + '#payment_fedapay_payments-head');
                                    location.reload();
                                }
                                else {
                                    pollTimer = setTimeout(self.pollForKeys(self), self.pollInterval);
                ...
                /**
                 * display fedapay simple path config section
                 */
                showfedapayConfig: function () {
                    this.$fedapaySimplepath.show();
                    this.$fedapaySpBack.hide();
                    if (this.$fedapayCredentialsHeader.hasClass('open')) {
                        this.$fedapayCredentialsHeader.click();
                    }
                },

                /**
                 * hide fedapay simple path config.
                 */
                hidefedapayConfig: function () {
                    this.$fedapaySimplepath.hide();
                    this.$fedapaySpBack.show();
                    if (!this.$fedapayCredentialsHeader.hasClass('open')) {
                        this.$fedapayCredentialsHeader.click();
                    }
                },
